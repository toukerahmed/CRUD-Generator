<x-layout>
        <div class="w3-container w3-padding-32 mt-5">
            <h1 class="w3-border-bottom w3-border-light-grey w3-padding-16 mb-4">Edit {{ $task1->id }}</h1>
        </div>
    <div class="card w3-black">
        <div class="card-body">
            <form action="{{ route('task1s.update', $task1) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="form-group">
                            ...
                            @foreach($options as $option)
                                <div class="form-check">
                                    <input type="radio"
                                        ...
                                        {{ old($field['name'], $task1[$field['name']]) == trim($option) ? 'checked' : '' }}>
                        ...
                        @else
                            <input type="text"
                                ...
                                name="{{ $field['name'] }}"
                                id="{{ $field['name'] }}"
                                value="{{ old($field['name'], $task1[$field['name']]) }}">
                        @endif
